...
void InterfaceNRF24::init(SPI_HandleTypeDef *spi_handle, const uint8_t *net_address, int len)
{
	if(!__instance)
...
InterfaceNRF24::InterfaceNRF24(SPI_HandleTypeDef *spi_handle, const uint8_t *net_addr, int len)
{
	mPacketsLost = 0;
	mSPI = spi_handle;
...
int InterfaceNRF24::transmit(const uint8_t *addr, uint8_t *payload, uint8_t length)
{
	int  tx_length = 0;
